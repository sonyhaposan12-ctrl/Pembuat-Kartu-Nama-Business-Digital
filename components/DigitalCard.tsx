...
export const DigitalCard: React.FC<DigitalCardProps> = ({ data }) => {
  ...
  const handleDownloadJPEG = async () => {
    ...
    if (frontCanvas && backCanvas) {
        ...
        const totalHeight = padding + labelHeight + cardHeight + gap + labelHeight + cardHeight + padding;
        ...
        if (ctx) {
            // Fill background with soft gray to make white cards pop
            ctx.fillStyle = '#f8fafc'; // Slate-50
            ctx.fillRect(0, 0, totalWidth, totalHeight);

            // Helper function to draw card with shadow and label
            const drawCard = (canvas: HTMLCanvasElement, yPos: number, label: string) => {
                 // Draw Label
                ctx.font = 'bold 28px Inter, sans-serif';
                ctx.fillStyle = '#334155'; // Slate-700
                ctx.textAlign = 'center';
                ctx.textBaseline = 'bottom';
                ctx.fillText(label, totalWidth / 2, yPos - 15);
                ...
                // Draw Image
                // Center the image if widths differ (though usually same)
                const xPos = padding + (cardWidth - canvas.width) / 2;
            ...
            // Draw Front
            drawCard(frontCanvas, padding + labelHeight, "Tampak Depan (Front)");
            
            // Draw Back
            drawCard(backCanvas, padding + labelHeight + cardHeight + gap + labelHeight, "Tampak Belakang (Back)");

            // Add Footer
            ctx.font = '16px Inter, sans-serif';
            ctx.fillStyle = '#94a3b8'; // Slate-400
            ctx.textAlign = 'center';
            ctx.fillText(`Generated by Seraphim Digital Card - ${data.company}`, totalWidth / 2, totalHeight - 20);

            const image = combinedCanvas.toDataURL("image/jpeg", 0.95);
            const link = document.createElement('a');
            link.href = image;
            link.download = `${data.name.replace(/\s+/g, '_')}_DigitalCard_Full.jpg`;
            link.click();
        }
    }
  };
...
<head>
    ...
    <style>
        ...
        /* Print Styles for cleaner physical printing of the HTML file */
        @media print {
            body { background: white !important; -webkit-print-color-adjust: exact; }
            .perspective-1000 { display: none; }
            .print-view { display: flex !important; flex-direction: column; align-items: center; gap: 20px; page-break-inside: avoid; }
            .print-card { 
...
<body class="bg-gray-100 flex flex-col items-center justify-center min-h-screen p-4">
    <!-- Interactive View -->
    <div class="perspective-1000 w-full max-w-md cursor-pointer group no-print" onclick="this.querySelector('.flip-card-inner').classList.toggle('flipped')">
